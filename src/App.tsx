...
export default function App() {
  ...
  function renderCalendar() {
    return (
      <Suspense fallback={<LazyPanelFallback icon={<CalendarDays className="h-5 w-5 text-moss" aria-hidden="true" />} title="Calendario" text="Preparando el calendario..." />}>
        <CalendarPanel
          ...
          isMonthlyAnniversary={isMonthlyAnniversary}
        />
      </Suspense>
    );
  }
  ...
  function renderChart() {
    return (
      <Suspense fallback={<LazyPanelFallback icon={<Thermometer className="h-5 w-5 text-coral" aria-hidden="true" />} title="Tus temperaturas recientes" text="Preparando el gráfico..." />}>
        <TemperatureChartPanel
          allChartDataLength={allChartData.length}
          canMoveChartLeft={canMoveChartLeft}
          canMoveChartRight={canMoveChartRight}
          chartData={chartData}
  ...
  function renderMap() {
    ...
    return (
      <Suspense fallback={<LazyPanelFallback icon={<HeartPulse className="h-5 w-5 text-moss" aria-hidden="true" />} title="Mapa del ciclo" text="Preparando el mapa..." />}>
        <MapPanel
          activeCycleIndex={activeCycleIndex}
          cycleWindows={cycleWindows}
          activeCycle={activeCycle}
          mapSelectedDay={mapSelectedDay}
          mapSelectedEntry={mapSelectedEntry}
          onSelectDate={setMapSelectedDate}
          onMoveCycle={moveCycle}
          isWaitingForInitialSync={isInitialCloudSyncSettling && !isDemoMode}
          stats={stats}
          entries={entries}
        />
      </Suspense>
...
export interface CycleWheelDay extends PhaseDay {
  isFuture: boolean;
  isToday: boolean;
}
...
function LazyPanelFallback({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
    <Panel>
      ...
      <div className="grid h-72 place-items-center rounded border border-dashed border-outline text-center text-sm text-ink/60">
